...
$popup.appendTo($('body'));let $validator=$form.validate();let showPopup=function(selector){$(selector).reveal({animation:animation,animationspeed:200,closeonbackgroundclick:true,dismissmodalclass:'close-reveal-modal',});};let download=function(){$form.prop('target','osdownloads-tmp-iframe-'+$form.prop('id'));$container=$('<div>').prop('id',prefix+'PopupIframe').addClass('reveal-modal').addClass('osdownloads-modal');$iframe=$('<iframe>').prop('name','osdownloads-tmp-iframe-'+$form.prop('id'));$close=$('<a class="close-reveal-modal">&#215;</a>');$iframe.appendTo($container);$close.appendTo($container);$container.appendTo($('body'));$form.submit();$container.on('reveal:close',function(){setTimeout(function(){$container.remove();},500);});$popup.trigger('reveal:close');setTimeout(function(){showPopup('#'+prefix+'PopupIframe');},500);};$this.on('click',function(event){event.preventDefault();event.stopPropagation();$btnContinue.prop('href',$this.prop('href'));showPopup('#'+popupElementId);$popup.on('reveal:close',function(){setTimeout(function(){$form[0].reset();$validator.resetForm();},500);});$btnContinue.off();$btnContinue.on('click',function(event){event.preventDefault();if($form.valid()){download();}});});$this.data('osdownloads-loaded',1);});}};});(function($){$('a[data-reveal-id]').on('click',function(event){event.preventDefault();var modalLocation=$(this).attr('data-reveal-id');$('#'+modalLocation).reveal($(this).data());});$.fn.reveal=function(options){var defaults={animation:'fadeAndPop',animationSpeed:300,closeOnBackgroundClick:true,dismissModalClass:'close-reveal-modal'};var options=$.extend({},defaults,options);return this.each(function(){var modal=$(this),topMeasure=parseInt(modal.css('top')),topOffset=modal.height()+topMeasure,locked=false,modalBg=$('.reveal-modal-bg');if(modalBg.length==0){modalBg=$('<div class="reveal-modal-bg" />').insertAfter(modal);modalBg.fadeTo('fast',0.8);}
