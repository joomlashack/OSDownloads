...

{
...

	{
		...
		$mc->listSubscribe($list_id, $email, $merge_vars, "html", false);
		if ($mc->errorCode)
		{
			//echo($mc->errorMessage);
		}
	...
	function linkdocument()
	{
		...
		$db->setQuery($query);
		$item = $db->loadObject();
		$this->setRedirect($item->documentation_link);
	}
	...
	function linkdemo()
	{
		...
		$db->setQuery($query);
		$item = $db->loadObject();
		$this->setRedirect($item->demo_link);
	}
	...
	function linksupport()
	{
		...
		$db->setQuery($query);
		$item = $db->loadObject();
		$this->setRedirect($item->support_link);
	}
	...
	function linkother()
	{
		...
		$db->setQuery($query);
		$item = $db->loadObject();
		$this->setRedirect($item->other_link);
	}
	...
	function test()
	{
		...
		$mc = new MCAPI($mailchimp_api);
		$merge_vars = array();		
		$mc->listSubscribe($list_id, "test2@vietactivegroup.com", $merge_vars);
		if ($mc->errorCode)
		{
			echo($mc->errorMessage);
		}
