...
function lockModal(){locked=true;}});};})(jQuery);(function osdownloadsClosure($){$(function osdownloadsDomReady(){$('.osdownloads-modal').appendTo($('body'));});$.fn.osdownloads=function osdownloads(options){var defaults={animation:'fade',elementsPrefix:'osdownloads',popupElementId:'osdownloadsRequirementsPopup',fieldsLayout:'block',};var options=$.extend({},defaults,options);if('block'===options.fieldsLayout){$tabs=$('.osdownloads-custom-fields-container ul.nav li a');if($tabs.length>0){$.each($tabs,function(index,elem){var $elem=$(elem),$panel=$($elem.attr('href'));$title=$('<h3>').addClass('osdownloads-fieldset-title').text($elem.text());$panel.before($title);$panel.addClass('active');});}
$('.osdownloads-custom-fields-container ul.nav').remove();}
if(this.length){return this.each(function osdownloadsEachElement(){var $this=$(this),$btnContinue=$('#'+options.elementsPrefix+'DownloadContinue'),$popup=$('#'+options.popupElementId),$errorAgreeTerms=$('#'+options.elementsPrefix+'ErrorAgreeTerms'),$errorInvalidEmail=$('#'+options.elementsPrefix+'ErrorInvalidEmail'),$errorShare=$('#'+options.elementsPrefix+'ErrorShare'),$fieldAgree=$('#'+options.elementsPrefix+'RequireAgree'),$fieldEmail=$('#'+options.elementsPrefix+'RequireEmail'),$groupEmail=$('#'+options.elementsPrefix+'EmailGroup'),$groupAgree=$('#'+options.elementsPrefix+'AgreeGroup'),$groupShare=$('#'+options.elementsPrefix+'ShareGroup'),$requiredEmailMessage=$('#'+options.elementsPrefix+'RequiredEmailMessage'),$requiredShareMessage=$('#'+options.elementsPrefix+'RequiredShareMessage'),directPage=$this.data('direct-page'),requireEmail=$this.data('require-email'),requireAgree=$this.data('require-agree')==1,requireShare=$this.data('require-share')==1,socialShared=false;var isValidForm=function(){var emailRegex=/^([A-Za-z0-9_\-\.\+])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,25})$/,errorElement=null,hasError=false,requireShare=$this.data('require-share')==1;if(requireAgree){if(!$fieldAgree.is(':checked')){hasError=true;$errorAgreeTerms.show();}else{$errorAgreeTerms.hide();}}
...
return true;};var showPopup=function(selector){$(selector).reveal({animation:options.animation,animationspeed:200,closeonbackgroundclick:true,dismissmodalclass:'close-reveal-modal',});window.setTimeout(function(){$('.osdownloads-custom-fields-container ul.nav li').first().find('a').trigger('click');},300);};var goToDirectPage=function(){if(directPage){window.location=directPage;}};var addQueryVarToUri=function(url,variable,value){if(url.indexOf('?')>-1){url+='&';}else{url+='?';}
url+=variable+'='+value;return url;};var download=function(){var url=$this.attr('href');$form=$('#'+$this.data('form-id')).attr('target','osdownloads-tmp-iframe-'+$this.data('form-id'));if(document.formvalidator.isValid($form[0])){$container=$('<div>').attr('id',options.elementsPrefix+'PopupIframe').addClass('reveal-modal').addClass('osdownloads-modal');$iframe=$('<iframe>').attr('name','osdownloads-tmp-iframe-'+$this.data('form-id'));$iframe.iframeAutoHeight({heightOffset:10});$close=$('<a class="close-reveal-modal">&#215;</a>');$iframe.appendTo($container);$close.appendTo($container);$container.appendTo($('body'));$form.submit();$container.on('reveal:close',function(){setTimeout(function timeoutRemoveIframePopup(){$container.remove();},500);});$popup.trigger('reveal:close');setTimeout(function timeoutShowPopup(){showPopup('#'+options.elementsPrefix+'PopupIframe');},500);}};$this.on('click',function downloadBtnOnClick(event){event.preventDefault();event.stopPropagation();var requireShare=$(this).data('require-share');if(requireEmail||requireAgree||requireShare){if(requireEmail!=0){if(requireEmail==1){$requiredEmailMessage.show();}else{$requiredEmailMessage.hide();}
