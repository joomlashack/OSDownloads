...
var $this=$(this),prefix=$this.data('prefix'),animation=$this.data('animation'),fieldsLayout=$this.data('fields-layout'),popupElementId=prefix+'_popup',$popup=$('#'+popupElementId),$btnContinue=$popup.find('.osdownloads-continue-button'),$errorAgreeTerms=$popup.find('.osdownloads-error-agree'),$errorInvalidEmail=$popup.find('.osdownloads-error-email'),$fieldAgree=$popup.find('.osdownloads-field-agree'),$fieldEmail=$popup.find('.osdownloads-field-email'),$groupEmail=$popup.find('.osdownloads-email-group'),$groupAgree=$popup.find('.osdownloads-group-agree'),directPage=$this.data('direct-page'),requireEmail=$this.data('require-email'),requireAgree=$this.data('require-agree')===1,$form=$popup.find('form');$popup.appendTo($('body'));$tabs=$popup.find('.osdownloads-custom-fields-container ul.nav li a');if($tabs.length>0){$.each($tabs,function(index,elem){var $tab=$(elem),panelSelector=$tab.attr('href'),$panel=$popup.find(panelSelector),uniqueId=$form.attr('id')+'-'+$panel.attr('id');$tab.attr('href','#'+uniqueId);$panel.attr('id',uniqueId);});}
...
let email=$fieldEmail.val().trim();switch(requireEmail){case 1:if(email===''||!emailRegex.test(email)){hasError=true;$errorInvalidEmail.show();}else{$errorInvalidEmail.hide();}
