...
function lockModal(){locked=true;}});};})(jQuery);(function osdownloadsClosure($){$.fn.osdownloads=function osdownloads(options){var defaults={animation:'fade',elementsPrefix:'osdownloads',popupElementId:'osdownloadsRequirementsPopup',fieldsLayout:'block',};var options=$.extend({},defaults,options);if(this.length){return this.each(function osdownloadsEachElement(){var $this=$(this),$popup=$('#'+options.popupElementId),$btnContinue=$('#'+options.elementsPrefix+'DownloadContinue'),$errorAgreeTerms=$('#'+options.elementsPrefix+'ErrorAgreeTerms'),$errorInvalidEmail=$('#'+options.elementsPrefix+'ErrorInvalidEmail'),$errorShare=$('#'+options.elementsPrefix+'ErrorShare'),$fieldAgree=$('#'+options.elementsPrefix+'RequireAgree'),$fieldEmail=$('#'+options.elementsPrefix+'RequireEmail'),$groupEmail=$('#'+options.elementsPrefix+'EmailGroup'),$groupAgree=$('#'+options.elementsPrefix+'AgreeGroup'),$groupShare=$('#'+options.elementsPrefix+'ShareGroup'),$requiredEmailMessage=$('#'+options.elementsPrefix+'RequiredEmailMessage'),$requiredShareMessage=$('#'+options.elementsPrefix+'RequiredShareMessage'),directPage=$this.data('direct-page'),requireEmail=$this.data('require-email'),requireAgree=$this.data('require-agree')==1,requireShare=$this.data('require-share')==1,prefix=$this.data('prefix'),socialShared=false,$form=$popup.find('form');$popup.appendTo($('body'));$tabs=$popup.find('.osdownloads-custom-fields-container ul.nav li a');if($tabs.length>0){$.each($tabs,function(index,elem){var $tab=$(elem),panelSelector=$tab.attr('href');$panel=$popup.find(panelSelector),uniqueId=$form.attr('id')+'-'+$panel.attr('id');$tab.attr('href','#'+uniqueId);$panel.attr('id',uniqueId);});}
if('block'===options.fieldsLayout){$tabs=$popup.find('.osdownloads-custom-fields-container ul.nav li a');if($tabs.length>0){$.each($tabs,function(index,elem){var $elem=$(elem),$panel=$($elem.attr('href'));$title=$('<h3>').addClass('osdownloads-fieldset-title').text($elem.text());$panel.before($title);$panel.addClass('active');});}
